...
- **🗂 탭 (Ctrl+T / Ctrl+W)** — 워크스페이스와 pane 사이에 **탭** 계층이 생겼다.
  ...
  **탭 하나가 화면 한 장**이고 그 안에서 기존처럼 분할할 수 있다. 안 보이는 탭도
  ...
  `Ctrl+Shift+N` 새 워크스페이스 · `Ctrl+Shift+W` 워크스페이스 닫기. `Ctrl+T`와
...
amux는 **워크스페이스 없이 빈 상태로 시작**합니다 — `+ 새 워크스페이스`를 누르면
**제목 입력창**이 떠서(포커스 자동 이동) 이름을 정한 뒤 Enter로 만들어집니다.
...
| 하고 싶은 것 | 방법 |
|---|---|
| 새 워크스페이스 | 사이드바 `+ 새 워크스페이스` → 제목 입력 후 Enter |
| 새 탭 | 탭바 오른쪽 `+` |
