...
> **상태: 10장의 수정은 2026-09-18 에 철회됐다. 그 수정이 반대 증상(친 글자가
> 사라짐)을 만들고 있었다 — 11장을 먼저 읽을 것.** 10장은 기록으로 남긴다.
>
...
> 거기 있는 것은 다시 하지 말 것.** 그리고 추측보다 먼저 `scripts/input-probe.py` 로
> **PTY 에 실제로 도착한 바이트**를 받을 것 (9장 끝의 실패 기록 참조 — `cat -v` 는 못 쓴다).
>
> 마지막 갱신: 2026-09-18 · 기준 커밋: `f77526d`
...
| 2026-08-08 | (미커밋) | **진짜 원인 수정** — 조합 시작 시 xterm 숨은 textarea 비우기 (10장) | ✅ 중복은 멎음 — 다만 **반대 증상을 만들었다**(11장) |
| 2026-09-18 | (이 커밋) | **10장 수정 철회** — 조합 시작 시 textarea 를 비우지 않는다 (11장) | 윈도우 사용자 보고로 착수. 사용자 확인 대기 |
...
## 11. 2026-09-18 — 10장의 수정이 만든 반대 증상: 친 글자가 사라진다

### 증상

사용자 보고(2026-09-18, 원문): **"지금 한글 타이핑을 프롬프트에 치면 방금 친 글자가
다음 글자로 인해 지워지고 있어"**

좁힌 조건 (사용자 확인):

- **터미널 칸에 직접 칠 때만.** 하단 입력창은 멀쩡하다.
- **한글에서만.** 영문은 멀쩡하다.
- 한 글자마다 **띄어쓰기를 하면 멀쩡하다** (사용자가 실제로 그렇게 쳐서 보고했다).

앞의 비대칭 두 개는 5장이 계속 물어보던 바로 그 항목이고, 세 번째가 결정적이다 —
**공백이 조합 사이를 벌리면 낫는다**는 것은 원인이 내용이 아니라 **시점**임을 말한다.

### 원인 — 10장의 우회가 xterm 의 "직전 글자 전송"을 빈 문자열로 만든다

xterm 은 조합이 끝난 글자를 `compositionend` **그 자리에서** 보내지 않는다. 브라우저가
textarea 값을 나중에 갱신하기 때문에 0ms 타이머를 걸어 두고 그때 잘라 보낸다.

```ts
// @xterm/xterm 6.0.0 — CompositionHelper._finalizeComposition(true)
this._isSendingComposition = true;
setTimeout(() => {
  currentCompositionPosition.start += this._dataAlreadySent.length;
  const input = this._isComposing
    ? this._textarea.value.substring(currentCompositionPosition.start, this._compositionPosition.start)
    : this._textarea.value.substring(currentCompositionPosition.start);
  if (input.length > 0) this._coreService.triggerDataEvent(input, true);
}, 0);
```

빠르게 치면 이 0ms 가 도는 **전에** 다음 글자의 `compositionstart` 가 도착한다. 그러면
`this._isComposing` 이 참이라 위의 첫 번째 가지를 타고, 끝 위치로 **"이번 조합이 시작된
지점"**(`_compositionPosition.start`)을 쓴다. 정상이라면 그 값은 직전 글자까지의 길이라
잘라낸 결과가 직전 글자 하나가 된다.

그런데 10장의 우회는 `compositionstart` 캡처 단계에서 **textarea 를 비운다.** 비운 뒤
xterm 의 `compositionstart()` 가 돌므로 `_compositionPosition.start = 0` 이 된다. 결국

```
input = value.substring(0, 0) === ""     →  input.length > 0 이 거짓  →  아무것도 안 보낸다
```

**직전 글자가 통째로 누락된다.** 조합 중에는 xterm 의 조합 표시(composition view)에 이미
보이고 있었으므로, 쓰는 사람 눈에는 "방금 친 글자가 다음 글자를 치는 순간 지워진다"로
보인다. 실제로는 지워진 것이 아니라 **PTY 로 보내진 적이 없다.**

한 글자씩 띄어 치면 낫는 이유도 이것이다. 공백은 조합이 아니라 그냥 키라, 다음
`compositionstart` 가 늦어져 0ms 타이머가 먼저 돌 틈이 생긴다.

### 10장의 우회는 애초에 필요 없었다

우회가 막으려던 것은 `_handleAnyTextareaChanges()` 가 누적분을 통째로 재전송하는
분기였다. 그런데 **설치된 xterm 6.0.0 에는 그 타이머에 가드가 있다.**

```ts
private _handleAnyTextareaChanges(): void {
  const oldValue = this._textarea.value;
  setTimeout(() => {
    if (!this._isComposing) {        // ← 조합이 시작됐으면 아무것도 하지 않는다
      ...
    }
  }, 0);
}
...
한글은 이 함수가 불린 직후(keyCode 229 keydown) 반드시 `compositionstart` 가 따라오므로
`_isComposing` 이 참이고, 문제의 세 분기는 **돌지 않는다.** 10장이 인용한 코드에는 이
가드가 빠져 있었다 — 인용이 부정확했던 것으로 보인다. `package.json` 은 스캐폴드
커밋(`54c96e6`) 이후 줄곧 `^6.0.0` 이고 `bun.lock` 도 `6.0.0` 고정이라, **10장을 쓰던
시점에도 같은 가드가 있었다.**

누적 자체도 해롭지 않다. 조합이 시작될 때의 길이를 기준점으로 잡아 그 뒤만 잘라 보내고,
포커스가 빠지면 xterm 이 스스로 비운다.

### 해결

`Terminal.svelte` 의 `resetImeBuffer` 와 그 `compositionstart` 캡처 리스너를 걷어냈다.
xterm 의 숨은 textarea 를 **밖에서 건드리지 않는다**는 것이 이 장의 결론이다. 그 값은
xterm 의 위치 계산(`_compositionPosition`)의 기준점이라, 비우는 순간 이미 날아간 계산이
조용히 빈 문자열을 내놓는다.

### 남는 위험과 확인 방법

10장이 관측했다는 "누적분 재전송"이 정말로 이 버전에서 일어난다면, 그 증상이 다시 나올
수 있다. 그때는 추측하지 말고 **먼저 증거를 받을 것**:

```
python scripts/input-probe.py     # pane 안에서 실행, 한글을 평소 속도로
```

- 친 글자가 **안 찍힘** → 전송 누락 (이 장의 증상)
- `'\x7f'` 가 섞임 → 백스페이스가 들어간 것 (다른 원인)
- 이미 친 문장이 **한 덩어리로** 찍힘 → 누적분 재전송 (10장의 증상)

이 도구가 9장 끝에서 "다음에 만들어야 한다"고 적어 둔 그것이다. 이번에 저장소에 넣었으니
다음 사람은 새로 만들지 말 것. `cat -v` 는 쓰지 말 것 (9장 끝 참조).
