...
- 발급자(CA)와 설정은 **`%APPDATA%\amux\`** 에 들어갑니다 (유닉스의 `~/.config/amux/` 자리).
  앱의 `session.json` 도 같은 곳입니다 — 모든 amux 도구가 한 자리를 봐야 하니까요.
  윈도우에서는 `$HOME` 을 **일부러 무시합니다.** 윈도우 자체에는 없는 값이고 Git Bash·MSYS
  가 자기가 띄운 셸에만 심어 주기 때문에, 그걸 따르면 bash 에서 켠 도구는
  `C:\Users\<당신>\.config` 를 보고 바로가기로 켠 앱은 `%APPDATA%` 를 봐서 한 PC 에 설정
  폴더가 두 개 생깁니다. 바꾸고 싶으면 `XDG_CONFIG_HOME` 을 명시적으로 지정하세요.
